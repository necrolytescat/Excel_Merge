(() => {
  ...
  function createExportChoice(view, row, side, action = "write") {
    const input = document.createElement("input");
    input.type = "checkbox";
    input.className = "export-row-choice";
    const decision = exportDecisionFor(view.sheet.id, row.key);
    input.checked = Boolean(
      decision
      && decision.action === action
      && (action === "delete" || decision.valueSide === side),
    );
    const enabled = Boolean(
      exportDraftFor()?.targetLayout
      && (action === "delete"
        ? side === "target" && row.status === "target_only" && row.targetValues
        : row[side + "Values"]),
    );
    ...
    input.setAttribute(
      ...
      action === "delete"
        ? "标记主键 " + row.key + " 删除目标行"
        : "选择主键 " + row.key + " 的" + sideLabel(side) + "数据写入",
    );
    input.addEventListener("click", (event) => event.stopPropagation());
    input.addEventListener("change", () => {
      setExportRowDecision(row, action, action === "delete" ? null : side);
    });
    const label = document.createElement("label");
    label.className = "diff-export-choice" + (action === "delete" ? " is-delete" : "");
    label.title = action === "delete" ? "删除目标行" : "选择该侧版本写入";
    label.append(input, document.createTextNode(action === "delete" ? "删" : "选"));
    return label;
  }
  ...
  function renderSheetNavigation(result, activeSheetId) {
    ...
    visibleSheetResults(result).forEach((sheet) => {
      ...
      const exportCounts = exportSelectionCounts(state.selectedPath, sheet.id);
      if (exportCounts.total) {
        const exportSummary = document.createElement("span");
        exportSummary.className = "sheet-export-summary";
        exportSummary.textContent = "写 " + exportCounts.write + " · 删 " + exportCounts.delete;
        meta.appendChild(exportSummary);
      }
