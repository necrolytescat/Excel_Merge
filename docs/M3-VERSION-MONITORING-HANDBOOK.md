...
- CSV 第 1 条逻辑记录是 `display_name`，第 2 条逻辑记录是 `field_name`；两者严格按同列位置配对，不从英文名猜中文；
- `scope=none` 不比较；
...
- 左侧导航列出契约中可以识别的有变化或公开错误工作簿，每项依次显示工作簿名、变化数和最终归因作者；顶部页签列出当前工作簿中有变化或错误的 Sheet；
- 工作簿最终归因作者从该工作簿的 `changes[]` 推导：选择 attribution Revision 最大的变化，Revision 相同时选择 `changed_at` 较晚者；存在 `unresolved`、没有变化或只有错误时显示“未知”。这不是 SVN 文件级最后提交人，不得替代字段级归因；
...
- 网格优先使用 `field_catalog` 按原 TableCsv 列顺序展示字段；表头第一行显示 CSV 第 1 条逻辑记录的中文展示名，第二行显示第 2 条逻辑记录的稳定字段名，主键列不重复进入普通字段列；
...
- 最终修改人筛选与“工作簿、修改人、错误”三项摘要放在同一区域，只改变展示，不改变报告 JSON、摘要统计或最终净值语义；报告不提供主键、字段和值搜索框。

新报告的 `field_catalog` 直接来自 Diff 已加载的起止 TableCsv 快照，不增加 SVN 调用或额外 Diff。旧报告缺少目录且含行新增/删除时，受控读取可以按报告固定起止 Revision 只读加载发生变化的工作簿和 Sheet，在内存中补齐并缓存；失败时退回原报告展示。该兼容过程不得改写历史 HTML/JSON、SHA、publication 或 `latest.html`，也不得改变最终净值、摘要统计和字段归因。
