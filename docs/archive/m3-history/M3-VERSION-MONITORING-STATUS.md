...
| 报告体验优化 | 已完成 | `codex/m3-report-experience` | `8db0abb`、`147a435`、`afebaf3` | Excel 式离线报告、中文双层表头和固定分支模糊匹配通过 |
| 增量性能优化 | 已完成 | `codex/m3-report-performance` | `aa26aca`、`c0dac7c`、`325eeaa` | 新旧语义一致，正式 Runner 切换增量引擎，真实影子约 5.21 倍加速 |

## 已完成的后续优化工作流

M3 功能验收完成后，两个优化工作流曾从同一个 `main` 基线并行启动，现均已完成：
...
- 摘要返修：移除无效的主键/字段/值搜索框；摘要只保留变化工作簿、修改人、错误，并把最终修改人筛选合并到同一区域
...
- 固定分支选择返修：原生下拉改为可输入模糊匹配组合框，支持鼠标、方向键、回车和 Esc；只有选中真实候选后才能创建任务，服务端 endpoint 契约不变
